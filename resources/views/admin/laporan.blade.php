    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex space-x-2">
                            <button type="button" onclick="openEditModal({{ $visitor->id }}, '{{ $visitor->name }}', '{{ $visitor->phone }}', '{{ $visitor->complaint }}')" 
                ...
                @empty
                <tr>
                    <td colspan="7" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">Tidak ada data pengunjung</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="addModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            <h3 class="text-lg leading-6 font-medium text-gray-900">Tambah Data Pengunjung</h3>
            <form action="{{ route('admin.laporan.store') }}" method="POST" class="mt-4 text-left">
                @csrf
                <div class="mb-4">
    ...
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="mt-3 text-center">
            ...
            <form id="editForm" method="POST" class="mt-4 text-left">
                ...
                <div class="mb-4">
                    <label for="edit_phone" class="block text-sm font-medium text-gray-700 mb-1">No. HP</label>
                    <input type="text" id="edit_phone" name="phone" 
                        class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full sm:text-sm border-gray-300 rounded-md">
                </div>
                <div class="mb-4">
                    <label for="edit_complaint" class="block text-sm font-medium text-gray-700 mb-1">Keluhan</label>
...
<script>
    ...
    // Modal Edit Data
    function openEditModal(id, name, phone, complaint) {
